...
core.register_alias("bucket:bucket_river_water", "bucket:bucket_water")
...
local function check_placeable(pos, name, bucket_type)
  ...
  if bucket_type == "bucket:bucket_lava" and pos.y > lava_limit then
    return true, core.chat_send_player(name, "Place lava below "..lava_limit)

  elseif (bucket_type == "bucket:bucket_water" or bucket_type == "bucket:bucket_river_water")
        and pos.y > water_limit then

    return true, core.chat_send_player(name, "Place water below "..water_limit)
  end

  if core.is_protected(pos, name) then
    core.log("action", (name ~= "" and name or "A mod").. " tried to  place "..bucket_type.. " at protected position ".. core.pos_to_string(pos))
    core.record_protection_violation(pos, name)
    return true
  end
  ...
  if not next(protectors) then
    return true, core.chat_send_player(name, "You don't own this area")
  end

  local meta, owner
  for n = 1, #protectors do
    meta = core.get_meta(protectors[n])
    owner = meta:get_string("owner") or ""

    if owner ~= name then
      core.log("action", (name ~= "" and name or "A mod").. " tried to place "..bucket_type.. " near ".. owner .."'s area ".. core.pos_to_string(protectors[n]) .. " ")
      return true, core.chat_send_player(name, "Too close to "..owner.."'s area: ".. core.pos_to_string(protectors[n]))
    end
  end
